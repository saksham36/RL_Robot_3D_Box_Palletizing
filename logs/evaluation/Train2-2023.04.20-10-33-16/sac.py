...
class SAC(object):
    def __init__(self, num_inputs, action_space, args, device):
        ...
        self.action_location = self.action_space[0]
        self.action_orientation = self.action_space[1]
        # Size of action in network: len(location)+ len(orientation) = LxW + 6
        self.action_size = len(self.action_location) + \
            len(self.action_orientation[0])
        # Action space shape: LxW + 6*number of blocks
        self.action_shape = len(
            self.action_location) + len(self.action_orientation[0]) * len(self.action_orientation)
            ...
            if self.automatic_entropy_tuning is True:
                self.target_entropy = - \
                    torch.prod(torch.Tensor(
                        self.action_size).to(self.device)).item()
                self.log_alpha = torch.zeros(
                    1, requires_grad=True, device=self.device)
                self.alpha_optim = Adam(
                    [self.log_alpha], lr=args.learning_rate)

            self.policy = GaussianPolicy(
                num_inputs, self.action_size, args.hidden_size, action_space).to(self.device)
    ...
    def select_action(self, state, evaluate=False):
        state = torch.FloatTensor(np.concatenate([state[0].flatten(), state[1].flatten()])).to(
